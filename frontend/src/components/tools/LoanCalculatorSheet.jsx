      ...
      <div className="space-y-6">
        ...
        <form onSubmit={calculateLoan} className="space-y-6">
          <div className="space-y-4">
            <h3 className="font-bold text-gray-900 dark:text-slate-100 border-b border-gray-100 dark:border-white/5 pb-2 flex items-center space-x-2">
              <span className="w-6 h-6 rounded-full bg-blue-100 dark:bg-indigo-900/30 text-blue-600 dark:text-indigo-400 flex items-center justify-center text-xs">1</span>
              <span>Thông tin Khế ước</span>
            </h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-slate-400 mb-2">Ngày giải ngân</label>
                <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full bg-gray-50 dark:bg-slate-800 border border-transparent focus:border-blue-500 dark:focus:border-indigo-500 rounded-xl px-4 py-3 outline-none font-medium text-gray-900 dark:text-white transition-all" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-slate-400 mb-2">Ngày trả nợ đầu tiên</label>
                <input type="date" value={firstPaymentDate} onChange={e => setFirstPaymentDate(e.target.value)} className="w-full bg-gray-50 dark:bg-slate-800 border border-transparent focus:border-blue-500 dark:focus:border-indigo-500 rounded-xl px-4 py-3 outline-none font-medium text-gray-900 dark:text-white transition-all" />
              </div>
            </div>
            <div>
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 dark:text-slate-400 mb-2">Kỳ hạn vay (Tháng)</label>
              <input type="number" value={termMonths} onChange={e => setTermMonths(e.target.value)} placeholder="VD: 120 (10 năm)" className="w-full bg-gray-50 dark:bg-slate-800 border border-transparent focus:border-blue-500 dark:focus:ring-indigo-500 rounded-xl px-4 py-3 outline-none font-medium text-lg text-gray-900 dark:text-white transition-all" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-slate-400 mb-2">Lãi ưu đãi (%/năm)</label>
                <input
                  type="text"
                  inputMode="decimal"
                  ...
                  placeholder="0"
                  className="w-full bg-gray-50 dark:bg-slate-800 border border-transparent focus:border-blue-500 dark:focus:border-indigo-500 rounded-xl px-4 py-3 outline-none font-medium text-lg text-gray-900 dark:text-white transition-all"
                />
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 dark:text-slate-400 mb-2">Kéo dài (Tháng)</label>
                <input type="number" value={promoMonths} onChange={e => setPromoMonths(e.target.value)} className="w-full bg-gray-50 dark:bg-slate-800 border border-transparent focus:border-blue-500 dark:focus:border-indigo-500 rounded-xl px-4 py-3 outline-none font-medium text-lg text-gray-900 dark:text-white transition-all" />
              </div>
            </div>
          </div>

          <div className="pt-2">
            <button type="button" onClick={() => setShowAdvanced(!showAdvanced)} className="flex items-center space-x-2 text-sm font-bold text-blue-600 dark:text-indigo-400 bg-blue-50 dark:bg-indigo-900/20 px-4 py-2 rounded-lg transition-all active:scale-95">
              <Settings2 size={16} />
          ...
          {showAdvanced && (
            <div className="space-y-6 animate-in fade-in slide-in-from-top-2">
              <div className="space-y-4">
                <h3 className="font-bold text-gray-900 dark:text-slate-100 border-b border-gray-100 dark:border-white/5 pb-2 flex items-center space-x-2">
                  <span className="w-6 h-6 rounded-full bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 flex items-center justify-center text-xs">2</span>
                  <span>Lãi suất Thả nổi</span>
                </h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-slate-400 mb-2">Lãi cơ sở (%)</label>
                    <input
                      type="text"
                      inputMode="decimal"
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-slate-400 mb-2">Biên độ (+%)</label>
                    <input
                      type="text"
                      inputMode="decimal"
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between border-b border-gray-100 dark:border-white/5 pb-2">
                  <h3 className="font-bold text-gray-900 dark:text-slate-100 flex items-center space-x-2">
                  ...
                  <button type="button" onClick={() => setShowPeriods(v => !v)} className={`text-xs font-bold px-3 py-1 rounded-full transition-all ${showPeriods ? 'bg-purple-600 dark:bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-slate-400'}`}>{showPeriods ? 'Đang bật' : 'Bật lên'}</button>
                </div>
                {showPeriods && (
                  <div className="space-y-3">
                    {periods.map((pd, idx) => (
                      <div key={idx} className="bg-purple-50 dark:bg-indigo-900/10 border border-purple-100 dark:border-indigo-900/20 rounded-xl p-3 space-y-2 transition-colors">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          <input
                            ...
                            className="bg-white dark:bg-slate-800 border border-transparent dark:border-white/5 rounded-lg px-3 py-2 text-sm font-bold shadow-sm dark:text-white outline-none"
                          ...
                          <div className="relative">
                            <input
                              ...
                              className="w-full bg-white dark:bg-slate-800 border border-transparent dark:border-white/5 rounded-lg px-3 py-2 pr-10 text-sm font-bold shadow-sm dark:text-white outline-none focus:ring-1 focus:ring-purple-400"
                    ...
                    <button type="button" onClick={() => { const last = periods[periods.length - 1]; const nextFrom = last ? parseInt(last.toMonth) + 1 : 1; setPeriods(prev => [...prev, { fromMonth: nextFrom, toMonth: nextFrom + 11, rate: last?.rate || toViDecimal(promoRate) || '', budget: last?.budget || extraPayment || '' }]); }} className="w-full py-2.5 border-2 border-dashed border-purple-300 dark:border-white/10 text-purple-600 dark:text-indigo-400 rounded-xl font-semibold text-sm active:scale-95 transition-all">Thêm Giai đoạn</button>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                <h3 className="font-bold text-gray-900 dark:text-slate-100 border-b border-gray-100 dark:border-white/5 pb-2 flex items-center space-x-2">
                  <span className="w-6 h-6 rounded-full bg-green-100 dark:bg-emerald-900/30 text-green-600 dark:text-emerald-400 flex items-center justify-center text-xs">4</span>
                  <span>Kế hoạch Tất toán sớm</span>
                </h3>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-gray-500 dark:text-slate-500 uppercase tracking-widest ml-1">Chuỗi phí phạt (%/năm)</label>
                  <input type="text" value={penaltyConfig} onChange={e => setPenaltyConfig(e.target.value)} className="w-full bg-white dark:bg-slate-800 border border-gray-200 dark:border-white/5 rounded-xl px-4 py-3 outline-none font-medium text-gray-900 dark:text-white transition-all" placeholder="VD: 3, 3, 3, 1, 0" />
          ...
          <button type="submit" className="w-full py-4 bg-blue-600 dark:bg-indigo-600 text-white font-bold rounded-2xl shadow-lg shadow-blue-200 dark:shadow-none mt-4 active:scale-95 transition-all">Chạy Mô phỏng Khoản Vay</button>
        </form>

        {result && (
          <div className="mt-8 pt-6 border-t border-gray-100 dark:border-white/5 animate-in fade-in slide-in-from-bottom-2">
            <div className="bg-gray-50 dark:bg-slate-900/50 rounded-2xl p-4 mb-6 text-center border border-gray-100 dark:border-white/5 transition-colors">
            ...
            <div className="space-y-3 mb-6">
              <div className="flex justify-between items-center bg-white dark:bg-slate-800 border border-gray-100 dark:border-white/5 p-3 rounded-xl shadow-sm transition-colors">
                <span className="text-gray-600 dark:text-slate-400 text-sm font-medium">Tổng Lãi (Gốc)</span>
                <span className="font-bold text-gray-900 dark:text-slate-100">{formatCurrency(result.baseTotalInterest)} đ</span>
              </div>
              <div className="flex justify-between items-center bg-blue-50 dark:bg-indigo-900/10 border border-blue-100 dark:border-indigo-900/20 p-3 rounded-xl shadow-sm font-bold transition-colors">
                <span className="text-blue-800 dark:text-indigo-400 text-sm">Thực trả (Lãi + Phí)</span>
                <span className="text-red-600 dark:text-rose-400">{formatCurrency(result.actualTotalInterest + result.totalPenalty)} đ</span>
              </div>
            </div>

            <div className="space-y-3">
              <button onClick={() => setIsAddRealLoanOpen(true)} className="w-full py-4 bg-gray-900 dark:bg-indigo-600 text-white font-black rounded-2xl shadow-xl dark:shadow-none flex items-center justify-center space-x-2 active:scale-95 transition-all">
                <Landmark size={20} />
                <span>Kích hoạt Hồ sơ vay thực tế</span>
              </button>
              <button onClick={() => setShowSchedule(!showSchedule)} className="w-full py-3 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300 font-bold rounded-xl flex items-center justify-center space-x-2 active:scale-95 transition-all">
            ...
            {showSchedule && schedule.length > 0 && (
              <div className="mt-4 bg-white dark:bg-slate-900 border border-gray-100 dark:border-white/5 rounded-xl overflow-x-auto shadow-sm no-scrollbar transition-colors">
                <table className="w-full text-[9px] text-right whitespace-nowrap">
                  ...
                  <tbody className="divide-y divide-gray-50 dark:divide-white/5 font-semibold text-gray-600 dark:text-slate-300">
                    {schedule.map((row) => (
