...
export function AddSavingsSheet({ isOpen, onClose, onSuccess }) {
  ...
  // Tái tục tự động khi đáo hạn (xử lý bằng cách tự quét lúc mở app).
  const [autoRenew, setAutoRenew] = useState(false);
  // Khi tái tục tự động: gộp cả lãi vào sổ mới (lãi kép) hay chỉ tái tục gốc.
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex items-center justify-between p-3 bg-blue-50 dark:bg-indigo-900/20 rounded-xl border border-blue-100 dark:border-indigo-900/30">
          <div className="space-y-0.5 pr-2">
            ...
            <p className="text-[10px] text-blue-600/80 dark:text-indigo-400/70 font-medium">Khi đến hạn, hệ thống tự mở một sổ mới (cùng lãi suất & kỳ hạn). Việc tái tục được xử lý khi bạn mở lại ứng dụng.</p>
          </div>
          <input
            type="checkbox"
            checked={autoRenew}
        ...
        {/* Phạm vi tái tục tự động: chỉ gốc hay cả gốc + lãi (lãi kép) */}
        {autoRenew && (
          <div className="flex items-center justify-between p-3 bg-indigo-50 dark:bg-indigo-900/10 rounded-xl border border-indigo-100 dark:border-indigo-900/30 ml-4">
            <div className="space-y-0.5 pr-2">
              ...
              <p className="text-[10px] text-indigo-600/80 dark:text-indigo-400/70 font-medium">Tái tục cả gốc + lãi (lãi kép). Nếu tắt, lãi dự kiến sẽ nhận về tài khoản nguồn.</p>
            </div>
            <input
              type="checkbox"
