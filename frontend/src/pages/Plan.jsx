...
export default function Plan() {
  ...
  return (
    <>
      <div className="px-4 lg:px-8 pt-safe pb-32 min-h-screen bg-gray-50 dark:bg-slate-950 transition-colors duration-300 max-w-7xl mx-auto">
        
        {/* --- HEADER & NAVIGATION --- */}
        <div className="mb-10">
          <div className="flex justify-between items-center px-1 mb-6">
            <h1 className="text-3xl lg:text-4xl font-black text-gray-900 dark:text-slate-100 mt-4 tracking-tight">Kế hoạch thu chi</h1>
            <button 
              onClick={() => setIsAddPlanOpen(true)}
              className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl shadow-lg lg:shadow-indigo-500/20 active:scale-95 transition-all mt-4 flex items-center space-x-2"
            >
              <Plus size={20} />
              <span className="text-sm font-black uppercase tracking-widest hidden sm:inline">Thêm mục mới</span>
            </button>
          </div>
          ...
          {planViewMode === 'monthly' && (
            <div className="flex items-center justify-center mt-8 px-1 space-x-8">
              <button 
                onClick={() => changeMonth(-1)}
                className="p-3 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-white/5 text-gray-400 dark:text-slate-500 hover:text-indigo-600 active:scale-90 transition-all"
              >
                <ChevronLeft size={24} />
              </button>
              <div className="text-center min-w-[120px]">
                <p className="text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-1">{year}</p>
                <h3 className="text-xl lg:text-2xl font-black text-indigo-600 dark:text-indigo-400">{monthLabel}</h3>
              </div>
              <button 
                onClick={() => changeMonth(1)}
                className="p-3 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-white/5 text-gray-400 dark:text-slate-500 hover:text-indigo-600 active:scale-90 transition-all"
        ...
        {/* --- GRID: INCOME & EXPENSE SECTIONS --- */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 mb-12">
          {/* --- SECTION: INCOME PLAN --- */}
          <section>
            <div className="flex items-center space-x-3 mb-6 px-1">
              <div className="w-1.5 h-6 bg-emerald-500 rounded-full shadow-sm shadow-emerald-500/40" />
              <h2 className="text-xl font-black text-gray-900 dark:text-slate-100 tracking-tight">{planViewMode === 'default' ? 'Dự thu mặc định' : 'Dự thu trong tháng'}</h2>
            </div>
            {loading ? (
              <div className="flex justify-center p-12">
                <div className="w-10 h-10 border-4 border-emerald-100 dark:border-emerald-900/30 border-t-emerald-600 rounded-full animate-spin"></div>
              </div>
          ...
          <section>
            ...
            {loading ? (
              <div className="flex justify-center p-12">
                <div className="w-10 h-10 border-4 border-blue-100 dark:border-blue-900/30 border-t-blue-600 rounded-full animate-spin"></div>
              </div>
        ...
        <section className="mb-10 pt-6 border-t border-gray-100 dark:border-white/5">
          <div className="flex items-center space-x-3 mb-8 px-1">
            <div className="w-1.5 h-6 bg-indigo-500 rounded-full shadow-sm shadow-indigo-500/40" />
            <h2 className="text-2xl font-black text-gray-900 dark:text-slate-100 tracking-tight">Tương lai & Tích luỹ</h2>
          </div>
            ...
            <div className="space-y-4">
              ...
              {showPlanTable && (
                <div className="bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-white/5 overflow-hidden animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="p-4 bg-gray-50 dark:bg-slate-800/50 border-t border-gray-100 dark:border-white/5 px-6">
                    ...
                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100 dark:border-white/5">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        />
                        <span className="text-[10px] text-gray-400 dark:text-slate-600 font-bold">₫</span>
                      </div>
                    </div>
                  </div>
                </div>
